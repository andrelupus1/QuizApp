...
class Account extends CI_Controller {
    ...
    //Recebe e-mail api TESTE
    public function api(){
        //$this->load->library('email');
        $urlAtivacao = base_url('/account/ativacao/?email=').$_GET['email'];//$emailUsuario;//envia email via get
        $subject = 'Ativação de conta de usuário - OftQuiz App';
        $message = '<p>Prezado,<br /> Clique no link baixo para ativar seu usuário:<br /></p>';
        $body = '<!DOCTYPE html>
        <head>
        ...
        $result = $this->email
            ...
            //->reply_to('')    // Optional, an account where a human being reads.
            ->to($_GET['email']) //recebe o email cadastrado
            ->subject($subject)
            ->message($body)
            ->send();
      //ver erros
    ...
    public function ativacao(){
        ...
        $resultado = $this->usuarios->ativacao($email);
            ...
            if($resultado){
               ...
               // print_r($resultado);
                if($this->usuarios->ativar($resultado)){
