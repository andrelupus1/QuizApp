...
class Usuarios_model extends CI_Model{
    ...
    public function getEmail($dados){
        ...
        $this->db->where('emailUsuario', $dados['emailUsuario']);
        //$this->db->where('senhaUsuario', $dados['senhaUsuario']);
        //$this->db->where('status', 2);//usuário aplicativo somente
        $query = $this->db->get();
    ...
    //ATIVAÇÃO VIA E-MAIL
    public function ativacao( $email ){//VERIFICA SE EXISTE O E-MAIL
    ...
    public function ativar($resultado = array()){//SE EXISTIR O CONTROLLER ATIVA
    ...
    //RECUPERA SENHA
     public function senhaEmail( $email ){//VERIFICA SE EXISTE O E-MAIL
        //$this->db->where("idusuarios","$id_usuario");
        $this->db->where("emailUsuario",$email);//consulta e-mail
        $resultado = $this->db->get("usuarios")->result_array();
        return $resultado;
    }
    public function resetar($resultado = array()){//SE EXISTIR O CONTROLLER RESETA
        $this->db->where("emailUsuario",$resultado['emailUsuario']);//
        $this->db->set('senhaUsuario', $resultado['senhaUsuario']);//Atualizando Senha
        return $this->db->update('usuarios');
    }
}